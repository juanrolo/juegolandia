...
public class Tablero {
	 ...
	 * 
	 * @param original
	 *            Tablero del cual realizar la copia
	 */
	...
	/**
	 * Devuelve a quién pertenece la casilla especificada. 0 está vacía 1
	 * pertenece al jugador 1 2 pertenece al jugador 2
	 */
            ...
            while ((!fin) && (j < m_numColumnas))
            {
                ...
                if(casilla != 0)
                {
                    ...
                    if(i+3 < m_numFilas)
                    {
                    	...
                    	if(j-3>=0)
                    	{
                    		if((m:tablero[i+1][j-1]==casilla)&&(m_tablero[i+2][j-2]==casilla&&(m_tablero[i+3][j+3]==casilla)))
                    	}
                    	ganador=casilla;
                    	fin = true;
                    }
                }
                //Realizar una b�squeda en la segunda diagonal
                if(j+3<m_numColumnas)
                {
 ...
 * @param jugador Indica el jugador que est� jugando, 1 para jugador 1, 2 para jugador 2
 */
 ...
 * Devuelve la fila en la que se le colocar�a la ficha y -1 en caso de error
 * @param columna
 * @return
 */
public int comprobarColumna(int columna)
{
	...
	//Comprueba la primera caslla libre de esa columna(si tiene)
	for(int i =0;(i<m_numFilas && !encontrado);i++)
	{
		if(m_tablero[i][columna]==0)
		{fila = i;
